/**
 * [fork] Vertical timeline — port of Sentinel NVR's `VerticalTimeline.tsx`
 * (Scrypted mechanics 1:1: fixed playhead at 35 %, native scroll = scrub
 * time-lapse, hold/release seeks, only the visible window is rendered). Changes
 * for HAPulse: API client + locale formatting come in as props, class colours
 * are the `--nvr-c-*` tokens, labels are translated.
 */
...
export interface TimelineProps {
  client: SentinelClient; t: TFunction; locale: Locale;
  camId: string;
  /** continuous range: rangeStart = oldest loaded midnight, rangeEnd = end of today */
  rangeStart: number; rangeEnd: number; clips: Clip[]; events: NvrEvent[]; motion: [number, number][];
  live: boolean; playhead: () => number | null; following: () => boolean; filterOff: Record<string, boolean>;
  onEvent: (ev: NvrEvent) => void; onGoLive: () => void; scrub: ScrubHandlers;
...
export function VerticalTimeline(p: TimelineProps) {
  const hhmm = (ts: number) => fmtTime(ts, p.locale), hhmmss = (ts: number) => fmtTimeSec(ts, p.locale), dayLbl = (ts: number) => dayLabel(ts, p.locale);
  const scroll = useRef<HTMLDivElement>(null);
  const [vh, setVh] = useState(600);
  const [px, setPx] = useState(0); // px per ms
  const [, tick] = useState(0);
  const VT = useRef({ user: 0, down: false, moved: false, previewing: false, vel: 0, smooth: null as number | null, lastC: null as { ts: number; at: number } | null, holdT: 0, relT: 0, lastCenterCb: 0, selfAt: 0, lastJumpLog: 0, idleAt: 0, selfTop: -1, backoffUntil: 0, lastForeignLog: 0, touching: false });
  useEffect(() => { const el = scroll.current!; const ro = new ResizeObserver(() => setVh(el.clientHeight || 600)); ro.observe(el); setVh(el.clientHeight || 600); return () => ro.disconnect(); }, []);
  const minPx = vh / (3 * DAY), maxPx = vh / (2 * 60 * 1000);
  useEffect(() => { if (!px && vh) setPx(vh / (2 * 3600 * 1000)); }, [vh, px]);
  // the playhead sits at 35 % of the viewport (Scrypted: 241 px of 687), not centred: more past below, a little future above
  const HEAD = 0.35;
  const PAD = Math.ceil(vh * HEAD), PAD_BOT = Math.ceil(vh * (1 - HEAD));
  ...
  const setTop = (el: HTMLDivElement, top: number, why: string) => {
    const v = VT.current; v.selfAt = Date.now(); el.scrollTop = top; v.selfTop = el.scrollTop;
    if (Math.abs(el.scrollTop - top) > 1.5) { // the browser refused (iOS during touch/momentum, or out of range) → don't retry every 100 ms
      v.backoffUntil = Date.now() + 400;
      if (Date.now() - v.lastForeignLog > 2000) { v.lastForeignLog = Date.now(); rlog('follow-stuck', { why, want: Math.round(top), got: Math.round(el.scrollTop), max: el.scrollHeight - el.clientHeight, ch: el.clientHeight, vh, touching: v.touching, vis: document.visibilityState }); }
    }
  };
  const scrollCenter = (ts: number, why = 'follow') => { const el = scroll.current; if (!el) return; if (Date.now() < VT.current.backoffUntil) return; const top = yFor(ts) - vh * HEAD; if (Math.abs(el.scrollTop - top) > 0.5) setTop(el, top, why); };
  // initial position: "now"; imperative jumps from the page
  const init = useRef(false);
  useEffect(() => { if (!px || init.current) return; init.current = true; scrollCenter(Date.now()); }, [px]);
  ...
  const onScroll = () => {
    ...
    if (!v.user) {
      ...
      if (Date.now() - v.lastForeignLog > 2000) { v.lastForeignLog = Date.now(); rlog('foreign-scroll', { d: Math.round(el.scrollTop - v.selfTop), touching: v.touching, live: p.live, vis: document.visibilityState }); }
      markUser(); v.down = v.touching;
    }
    else if (Date.now() - v.selfAt < 80 && Math.abs(el.scrollTop - v.selfTop) <= 1.5) { v.lastC = { ts: centerTs(), at: Date.now() }; return; } // range-end compensation, not the user
    v.user = Date.now(); v.moved = true;
    if (v.previewing) { v.previewing = false; p.scrub.begin(); }
    const now = Date.now(), c = centerTs();
    if (v.lastC) { const dt = Math.max(16, now - v.lastC.at); v.vel = 0.7 * v.vel + 0.3 * ((c - v.lastC.ts) / dt * 1000); }
    v.lastC = { ts: c, at: now };
  ...
  const vis = useMemo(() => p.events.filter(e => !p.filterOff[classOf(e)]).slice().sort((a, b) => b.timestamp - a.timestamp), [p.events, p.filterOff]);
  // visible window (plus one screen each side) — everything else is not rendered
  const top = scroll.current?.scrollTop || 0;
  const winStart = tsForY(top + 2 * vh), winEnd = tsForY(top - vh);
    ...
    <div className="nvr-vtl vtl-wrap">
      <div className="vtl-scroll" ref={scroll} onScroll={onScroll} onPointerDown={down} onTouchStart={down}>
        <div className="vtl-content" style={{ height: span * px + PAD + PAD_BOT }}>
          ...
          {axis.map(t => <div key={t} className="vaxis data-font" style={{ top: yFor(t) }}>{hhmm(t)}</div>)}
          {midnights.map(t => <div key={'d' + t} className="vday" style={{ top: yFor(t) }}><span className="data-font">{dayLbl(t)}</span></div>)}
          {runs.map((r, i) => inWin(r.s, r.e) && <div key={i} className="vseg" style={{ top: yFor(r.e), height: Math.max(2, (r.e - r.s) * px) }} />)}
          {p.motion.map((m, i) => inWin(m[0], m[1]) && <div key={i} className="vmot" style={{ top: yFor(m[1]), height: Math.max(4, (m[1] - m[0]) * px) }} />)}
          {vis.map(ev => {
            if (!inWin(ev.timestamp)) return null;
            const y = yFor(ev.timestamp); const k = classOf(ev);
            const thumb = y - lastThumbY >= thumbGap && px > vh / (8 * 3600 * 1000);
            if (thumb) lastThumbY = y;
            return (
              <div key={ev.id}>
                <button className={'vev' + (thumb ? '' : ' vev--minor')} style={{ top: y, background: `var(--nvr-c-${k})` }} onClick={() => p.onEvent(ev)} aria-label={`${classLabel(p.t, k)} ${hhmmss(ev.timestamp)}`} title={hhmmss(eventPlayTs(ev))} />
                {thumb && <>
                  <div className="vlink" style={{ top: y }}><span className="vlink-badges"><EventBadges ev={ev} t={p.t} /></span><i /></div>
                  <button className="vthumb" style={{ top: y }} onClick={() => p.onEvent(ev)}>
                    <img src={p.client.eventThumbUrl(p.camId, ev.timestamp)} alt="" loading="lazy" />
                    <span className="tt data-font">{hhmmss(ev.timestamp)}</span></button>
                </>}
              </div>
            );
          })}
          {inWin(now) && <><div className="vlive-line" style={{ top: yFor(now) }} /><button className="vlive-chip" style={{ top: yFor(now) }} onClick={p.onGoLive}>{p.t('nvr.live')}</button></>}
        </div>
      </div>
      <div className="vcenter"><span className="vcenter-t data-font" data-ts={Math.round(center)}>{centerIsToday ? '' : dayLbl(center) + ' '}{hhmmss(center)}</span></div>
      <div className="vtl-zoom">
        <button type="button" className="tool" onClick={() => zoomStep(1.7)} aria-label={p.t('nvr.timeline.zoomIn')}>+</button>
        <button type="button" className="tool" onClick={() => zoomStep(1 / 1.7)} aria-label={p.t('nvr.timeline.zoomOut')}>−</button>
      </div>
      {!p.live && <button type="button" className="livejump" onClick={p.onGoLive}>↑ {p.t('nvr.live')}</button>}
    </div>
  );
}
