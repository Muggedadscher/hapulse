/**
 * [fork] WebRTC session over the plugin's WebSocket signaling. Port of Sentinel
 * NVR's `ui/src/player/webrtc.ts` (docs/API.md §6) with the API client injected
 * instead of derived from `location` — HAPulse runs on another origin.
 *
 * Original notes: WebRTC session over the plugin's WebSocket signaling — a 1:1 port of the
 * battle-tested vanilla logic (webui/js/streaming.ts): options first,
 * createLocalDescription/setRemoteDescription RPCs, trickle ICE, stream
 * composed from ALL receivers (Safari attached an audio-only stream forever
 * when the audio track arrived first), fresh offer on renegotiation, dec-check
 * after connect with a one-time TURN-relay retry on a media blackhole.
 */
