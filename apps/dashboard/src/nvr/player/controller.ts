/**
 * [fork] PlayerController — the whole playback brain of the NVR. Port of
 * Sentinel NVR's `ui/src/player/controller.ts` (itself a 1:1 port of the proven
 ...
 * and reports state through `onState`. React only renders what it says.
 *
 ...
 */
import type { SentinelClip as Clip } from '@hapulse/core';
...
type Opts = { onState: (s: PlayerState) => void; onClipsRefresh: () => Promise<Clip[]>; };
...
export class PlayerController {
  ...
  constructor(api: SentinelClient, opts: Opts) { this.api = api; this.opts = opts; setRlogClient(api); }
  ...
  setCamera(camId: string, name: string): void {
    ...
    let ar = ''; try { ar = localStorage.getItem('hapulse-nvr-ar-' + camId) || ''; } catch { /* ignore */ }
  ...
  private setAspect(w: number, h: number): void {
    ...
    try { localStorage.setItem('hapulse-nvr-ar-' + this.camId, ar); } catch { /* ignore */ }
  }
  ...
  scrubMove(centerTs: number, vel: number, smoothTs: number): void {
