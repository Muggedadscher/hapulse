/**
 * [fork] Sentinel NVR — DOM-free data model + helpers for the native HAPulse
 * integration.
 *
 * Mirrors `docs/API.md` of the Sentinel NVR plugin (the binding contract). Only
 * pure functions live here: URL/setup parsing, event classification, playback
 * start heuristics, the storage forecast the Sentinel status page computes, and
 * the clip-run merge the timeline draws. Everything that touches fetch/DOM sits
 * in `apps/dashboard/src/nvr/`.
 *
 * The whole native integration is designed to be removable: this file, the
 * `apps/dashboard/src/nvr/` directory, `pages/Nvr.*` and a handful of
 * `[fork]`-marked lines (see `docs/NVR-INTEGRATION.md`).
 */
